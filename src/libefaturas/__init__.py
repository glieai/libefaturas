"""
API pública da biblioteca libefaturas.
...
def test_connection(
    *,
    ...
    """
    Teste simplificado de ligação ao webservice da AT.

    Parâmetros:
      - username, password: credenciais Portal das Finanças
      - public_key_path: caminho para a chave pública/certificado da AT (PEM/CER)
      - endpoint: URL do endpoint SOAP (faturas ou séries)
      - client_cert_path / client_key_path: cert + key do cliente
      - ca_cert_path: bundle de CAs (opcional)
      - timeout: timeout do pedido HTTP
      - service: "faturas" ou "series"

    Devolve um dicionário com:
      - username_token_ok: bool
      - tls_ok: bool
      - http_status: int | None
      - soap_fault_code: str | None
      - soap_fault_string: str | None
      - raw_response_snippet: str | None
      - error: str | None
    """
    result: Dict[str, Any] = {
    ...
    # 2) Construir envelope SOAP mínimo dependendo do serviço
    service_norm = (service or "faturas").lower()

    if service_norm == "series":
        # Body real: consultarSeries (todos os filtros opcionais omitidos)
        body_xml = (
            "<S:Body>"
        ...
        # Body dummy para faturas (apenas handshake)
        body_xml = (
            "<S:Body>"
    ...
    # 3) Preparar certificado cliente
    if client_key_path:
    ...
    # 5) Tentar extrair SOAP Fault (se existir)
    try:
